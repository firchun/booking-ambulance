    ...
    <div class="dashboard-content card d-flex flex-column p-4 bg-white">
        ...
        <div class="form-action d-flex flex-row w-100 justify-content-between pb-4">
            <div class="buttons d-flex flex-row gap-2">
                <a href="{{ route('pengguna.create') }}" class="btn btn-primary"><i class="uil uil-plus"></i>&nbsp;Tambah
                    Pengguna</a>
            </div>
        </div>
                    ...
                    @foreach ($data as $row)
                        <tr>
                            ...
                            <td class="text-center">
                                <div
                                    class="btn btn-{{ $row->status == 'diproses' ? 'warning' : ($row->status == 'disetujui' ? 'success' : 'danger') }} btn-sm">
                                    {{ $row->status }}</div>
                            </td>
                            <td class="text-center">
                                <div class="buttons">
                                    ...
                                    <button onclick="triggerDelete('{{ route('pengguna.destroy', $row->id) }}')"
