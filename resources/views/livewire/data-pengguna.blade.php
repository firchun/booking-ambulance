    ...
    <div class="dashboard-content card d-flex flex-column p-4 bg-white">
        ...
        <div class="form-action d-flex flex-row w-100 justify-content-between pb-4">
            <div class="buttons d-flex flex-row gap-2">
                <a href="{{ route('pengguna.create') }}" class="btn btn-primary"><i
                        class="uil uil-plus"></i>&nbsp;Tambah Pengguna</a>
                <div class="w-25">
            ...
            <tbody>
                ...
                @foreach ($data as $row)
                    <tr>
                        ...
                        <td>{{ $row->email }}</td>
                        <td>
                            <button wire:click="$dispatch('triggerEdit',{{ $row->id }})"
                                class="btn btn-{{ $row->status == 'diproses' ? 'warning' : ($row->status == 'disetujui' ? 'success' : 'danger') }} btn-sm">{{ $row->status }}</a>
                        </td>
