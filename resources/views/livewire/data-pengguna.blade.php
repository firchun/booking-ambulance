<div class="container-fluid d-flex flex-column align-items-end p-0 bg-white min-vh-100 p-4">
    ...
    <div class="dashboard-content card d-flex flex-column p-4 bg-white">
        <div class="dashboard-title">
            <i class="uil uil-user"></i>&nbsp;Data Pengguna
        </div>
        <hr class="pb-4">
        <div class="form-action d-flex flex-row w-100 justify-content-between pb-4">
            <div class="buttons d-flex flex-row gap-2">
                ...
                <div class="w-25">
                    <input type="text" name="" wire:model.live="search" id="" class="form-control" placeholder="Cari...">
                </div>
            </div>
            <div class="form">
        ...
        <table class="table table-responsive table-bordered align-middle">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $row->email }}</td>
                    <td>
                    <button wire:click="$dispatch('triggerEdit',{{$row->id}})" class="btn btn-{{ $row->status == 'diproses' ? 'warning' : ($row->status == 'disetujui' ? 'success' : 'danger') }} btn-sm">{{ $row->status }}</a>
                    </td>
                    <td class="text-center">
                        <div class="buttons d-flex gap-2 justify-content-center">
                            <a href="{{ route('pengguna.edit', $row->id) }}" class="btn btn-warning btn-sm"><i class="uil uil-edit"></i></a>
                            <button wire:click="$dispatch('triggerDelete',{{$row->id}})" class="btn btn-danger btn-sm"><i class="uil uil-trash-alt"></i></button>
                        </div>
                    </td>
                </tr>
                @endforeach
...
<script type="text/javascript">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        @this.on('triggerEdit', id => {
            Swal.fire({
                title: 'Edit Status Pengguna',
                text: 'Silahkan pilih status pengguna yang baru',
                input: 'select',
                inputOptions: {
                    'diproses': 'Diproses',
                    'disetujui': 'Disetujui',
                ...
                if (result.value) {
                    @this.call('update', id, result.value)
                    Swal.fire({
